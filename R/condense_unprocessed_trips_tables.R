...
#' @export
condense_unprocessed_trips_tables = function(folder_location, cores, border_xing_table){
  ...
  # folder_location = "//geoatfilpro1/cadd3/inrix_data/trips_usa_tx_202202_wk1/date=2023-11-14/reportId=166997/v1/data/trips"
  # cores = 60
  ...
  dt_files = data.table(
    files = list.files(
      here::here(folder_location),"parquet"
    ))
  ...
  arrow::write_parquet(
    temp_file_dt
    ,here::here(
      "//geoatfilpro1/cadd3/inrix_data/oneoffs"
      ,"oneoff_condensed_trips.parquet"
    )
  )

  temp_file_agg %>%
    .[,.(count = .N), by = .(start_lat, start_lon)]
